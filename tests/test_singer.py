...
class TestSinger(unittest.TestCase):
    ...
    def test_parse_message_record_with_version_good(self):
        message = singer.parse_message(
            '{"type": "RECORD", "record": {"name": "foo"}, "stream": "users", "version": 2}'  # noqa: E501
        )
        self.assertEqual(
            message,
            singer.RecordMessage(
                record={"name": "foo"}, stream="users", version=2
            ),
        )

    def test_parse_message_record_naive_extraction_time(self):
        with self.assertRaisesRegex(
            ValueError, "must be either None or an aware datetime"
        ):
            message = singer.parse_message(  # noqa: F841
                '{"type": "RECORD", "record": {"name": "foo"}, "stream": "users", "version": 2, "time_extracted": "1970-01-02T00:00:00"}'  # noqa: E501
            )

    def test_parse_message_record_aware_extraction_time(self):
        message = singer.parse_message(
            '{"type": "RECORD", "record": {"name": "foo"}, "stream": "users", "version": 2, "time_extracted": "1970-01-02T00:00:00.000Z"}'  # noqa: E501
        )
    ...
    def test_extraction_time_strftime(self):
        """Test that we're not corrupting timestamps with cross platform parsing. (Test case for OSX, specifically)"""  # noqa: E501
    ...
    def test_parse_message_schema_good(self):
        message = singer.parse_message(
            '{"type": "SCHEMA", "stream": "users", "schema": {"type": "object", "properties": {"name": {"type": "string"}}}, "key_properties": ["name"]}'  # noqa: E501
        )  # nopep8
        self.assertEqual(
            message,
    ...
    def test_parse_message_schema_missing_stream(self):
        with self.assertRaises(Exception):
            message = singer.parse_message(  # noqa: F841
                '{"type": "SCHEMA", "schema": {"type": "object", "properties": {"name": {"type": "string"}}}, "key_properties": ["name"]}'  # noqa: E501
            )
    ...
    def test_parse_message_schema_missing_key_properties(self):
        with self.assertRaises(Exception):
            message = singer.parse_message(  # noqa: F841
                '{"type": "SCHEMA", "stream": "users", "schema": {"type": "object", "properties": {"name": {"type": "string"}}}}'  # noqa: E501
            )
    ...
    def test_parse_message_batch_good(self):
        message = singer.parse_message(
            '{"type": "BATCH", "stream": "users", "filepath": "/tmp/users0001.jsonl", "format": "jsonl", "time_extracted": "1970-01-02T00:00:00.000Z"}'  # noqa: E501
        )
        self.assertEqual(
            message,
            singer.BatchMessage(
                ...
                time_extracted=dateutil.parser.parse(
...
class TestParsingNumbers(unittest.TestCase):
    def create_record(self, value):
        raw = (
            '{"type": "RECORD", "stream": "test", "record": {"value": '
            + value
            + "}}"
        )
