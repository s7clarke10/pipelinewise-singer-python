...
def make_expected_metadata(base_obj, dict_of_extras):
    ...
    return [
        {"metadata": metadata_value, "breadcrumb": ()},
        {
            "metadata": {
                "inclusion": "available",
            },
...
class TestStandardMetadata(unittest.TestCase):
    ...
    def test_standard_metadata(self):
        ...
        test_value = get_standard_metadata(
            test_schema, tap_stream_id, test_kp, test_rk, test_rm
        )

        expected_metadata = make_expected_metadata(
            schema_present_base_obj,
            {
                "table-key-properties": ["id"],
                "forced-replication-method": "INCREMENTAL",
